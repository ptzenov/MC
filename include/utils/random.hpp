...
void draw_without_replacement(RandomIt first, RandomIt last, OutputIt out,
			      int num_elems) {
	...
	for (int i = 0; i < num_elems; i++) *(out + i) = *(first + indices[i]);
};

template <typename RandomIt>
void shuffle(RandomIt first, RandomIt last, int num_elems = -1) {
	auto seq_size = std::distance(first, last);
	if (num_elems == -1) num_elems = seq_size;
